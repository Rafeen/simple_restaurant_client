...
use crate::random_generators::{
    generate_order_items, generate_random_item_for_update, get_random_id_for_generated_items,
...
async fn main() {
    ...
    for table in 1..=total_tablet_count {
        pool.execute(move || {
            ...
            if table_id > 13 {
                match runtime.block_on(async { create_table(Table { id: table_id }).await }) {
                    Ok(_) => {}
            ...
            for _ in 1..=MAX_REQUEST_PER_THREAD {
                ...
                match request_type {
                    1 => match runtime.block_on(async {
                        create_order_for_table(table_id, order_item.clone()).await
                    }) {
                        Ok(_) => {}
                        Err(e) => println!("Error : {}", e),
                    },
                    ...
                    5 => match runtime
                        .block_on(async { get_remaining_orders_for_table(table_id).await })
                    {
                        Ok(_) => {}
                        Err(e) => println!("Error : {}", e),
                    },
                    6 => match runtime
                        .block_on(async { get_all_order_items_by_table_id(table_id).await })
                    {
                        Ok(_) => {}
                        Err(e) => println!("Error : {}", e),
                    },
...
async fn create_order_for_table(
    table_id: i32,
...
async fn get_table_order_by_item(table_id: i32, item_id: i32) -> Result<(), Error> {
    ...
    println!("[GET] : {}", url);
    println!(
        "[GET] order item {} for table {} : {:?}",
        item_id, table_id, msg
    );
    Ok(())
}
...
async fn get_remaining_orders_for_table(table_id: i32) -> Result<(), Error> {
    ...
    println!("[GET] : {}", url);
    println!(
        "[GET] remaining order items for table {} : {:?}",
        table_id, msg
    );
    Ok(())
}
